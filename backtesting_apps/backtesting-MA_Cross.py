...
#df=tv.get_hist('ADAUSD','BINANCE',Interval.in_daily,n_bars=5000)
#modified = df.reset_index()

btc_usd_data=tv.get_hist('ADAUSD','BINANCE',Interval.in_1_hour,n_bars=100000)
...
for i in modified_hourly.index: 
    ...
    if (modified_hourly['close'][i] > modified_hourly['MA200'][i]):
        ...
        if(modified_hourly['MA50'][i] > modified_hourly['MA100'][i] and modified_hourly['MA50'][i-1] < modified_hourly['MA100'][i-1]):
            cross = 1
            #print('cross',modified_hourly['datetime'][i])
        
        if(cross == 1 and modified_hourly['open'][i] <= modified_hourly['MA50'][i] <= modified_hourly['close'][i]):
            cross = 0
            touch = 1
            #print('touch', modified_hourly['datetime'][i])

        if(touch == 1 and in_position == 0):
            if(modified_hourly['close'][i] > modified_hourly['MA50'][i] and modified_hourly['open'][i] <  modified_hourly['close'][i]):
                in_position = 1
                buy_price = modified_hourly['close'][i]
                touch = 0
                print('buy_price', buy_price)
                atr_buy = modified_hourly['ATR'][i]
                buy.append(modified_hourly['close'][i])
                print('buy', modified_hourly['datetime'][i])
        
    if(modified_hourly['close'][i] <= (buy_price - 8 * atr_buy) and in_position == 1):
        in_position = 0
        sell_price = modified_hourly['close'][i]
        print('sell_price', sell_price)
        print('sell', modified_hourly['datetime'][i])
        sell.append(modified_hourly['close'][i])

    if(modified_hourly['close'][i] >  (buy_price + 4*(8 * atr_buy)) and in_position == 1):
        in_position = 0
        sell_price = modified_hourly['close'][i]
        print('sell_price', sell_price)
        print('sell', modified_hourly['datetime'][i])
...
#---------------------------------------------------
    ...
    if (modified_hourly['close'][i] > modified_hourly['MA200'][i]):
        ...
        if(cross == -1 and modified_hourly['close'][i] <= modified_hourly['MA50'][i] <= modified_hourly['open'][i]):
            cross = 0
            touch = -1
        ...
        if(touch == -1 and in_position == 0):
            if(modified_hourly['close'][i] < modified_hourly['MA50'][i] and modified_hourly['open'][i] >  modified_hourly['close'][i]):
                ...
                sell_price = modified_hourly['close'][i]
                touch = 0
                print('sell_short_price_', sell_price)
                atr_short = modified_hourly['ATR'][i]
                sell.append(modified_hourly['close'][i])
                print('sell_short', modified_hourly['datetime'][i])

    if(modified_hourly['close'][i] >= (sell_price + 4 * atr_short) and in_position == -1):
        in_position = 0
        buy_price = modified_hourly['close'][i]
        print('buy_long_price', sell_price)
        print('buy_long', modified_hourly['datetime'][i])
        buy.append(modified_hourly['close'][i])

    if(modified_hourly['close'][i] <  (sell_price - 4*(4 * atr_short)) and in_position == -1):
        in_position = 0
        buy_price = modified_hourly['close'][i]
        print('sell', modified_hourly['datetime'][i])
        print('buy_long_price', buy_price)
        print('buy_long', modified_hourly['datetime'][i])
        buy.append(modified_hourly['close'][i])
