...
import votes_original as votes
...
RSI_PERIOD = 14
RSI_OVERSOLD = 30 #treshold
RSI_OVERBOUGHT = 70 #treshold
...
#it allows the user to input their symbol and if the symbol was wrong it lets you try again

# file = open('path', 'a') #w - write, r - read, a - append
# file.write('\n write data')
#------------------------------------------------
...
def order(side, quantity, symbol, order_type = ORDER_TYPE_MARKET):
    try:
        print("sending order")
        order = client.futures_create_order(symbol = symbol, side = side, type = order_type, quantity = quantity, recvWindow = 120000)
        print(order)
        return True     
...
#     k = i
#     j = i
#     close_h = df["Hourly_Closes"][i]
#     close_h1 = df["Hourly_Closes"][1]
#     rsi = talib.RSI(close_h, RSI_PERIOD)
...
#     if (8 <= j <= 10):
#         j = j + 8
#     ax1_h = plt.subplot(5,5,j+1)
#     #ax1_h.plot(date_h,close_h,color='black')
#     ax1_h.plot(close_h,color='black')
#     ax1_h.grid(True)
...
#     if (8 <= k <= 10):
#          k = k + 12
#     ax2_h = plt.subplot(5,5,k)
#     # ax1_h.plot(rsi,color='black')
#     # # ax2_h.plot(close_h1,color='black')
...
def main():
    ...
    def websocket_thread():
        ...
        ws.send(json.dumps({
                    ...
                    #"params": ["btcusdt@kline_5m"], #"btcusdt@kline_1d"
                    "id": 1,}))
...
#-----------------------------------------------------------------------------------
 ...
 #-----------------------------------------------------------------------------------                               
                ...
                if (('socket is already closed.' in e) == True):
                    print(e)
                    print('-----------------')
